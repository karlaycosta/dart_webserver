...
void main() async {
  ...
  app.get('/teste', (Request request) {
    return Response(
      200,
      body: '[{"nome1": "Deriks Karlay"}, {"nome2": "Deriks Karlay"}]',
  ...
  app.get('/alimentosfiltrados/<predicado>',
      (Request request, String predicado) async {
...
Future<String> getAlimentos(Database db) async {
  final res = await db.rawQuery('select * from alimentos limit 1000');
  return jsonEncode(res);
}

Future<String> getAlimentosFiltrados(Database db, String predicado) async {
  predicado = removeDiacritics(predicado);
